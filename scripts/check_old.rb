...
def check_barcodes flowcell_data
	...
	if flowcell_data['samples'].size < 1
		...
		flowcell_data['samples'].each do |sample|
			...
			if ['ILL', 'Illumina TruSeq', 'Illumina','NEB','Rubicon R40048','Rubicon_Dual','BIOO','BiooScientific','BioScientific','BiooSci_Fake','BiooSci_Trimmed'].include? sample['indexType']
			else
				puts red("indexType is #{sample['indexType']}, lane #{sample['landID']}")
			end
		end
