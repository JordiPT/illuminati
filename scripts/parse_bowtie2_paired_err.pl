...
foreach my $file (@files)
{
	...
	while(<CURRENT>)
	{
		...
		{
			$h{$file}{bam_file} = $file;
			$h{$file}{pct_overall} = 0;
			$h{$file}{single_aligned_multi} = 0;
			$h{$file}{single_aligned_once} = 0;
			$h{$file}{pct_single} = 0;
			$h{$file}{single} = 0;
			$h{$file}{total_aligned}=0;
			$h{$file}{pct_multi}=0;
			$h{$file}{multi}=0;
			$h{$file}{pct_failed}=0;
			$h{$file}{failed}=0;
			$h{$file}{pct_aligned}=0;
			$h{$file}{aligned}=0;
			$h{$file}{proc}=0;
		}
	}
...
{
	print "$h{$f}{bam_file}\t$h{$f}{proc}\t$h{$f}{aligned}\t$h{$f}{pct_aligned}\t$h{$f}{failed}\t$h{$f}{pct_failed}\t$h{$f}{multi}\t$h{$f}{pct_multi}\t$h{$f}{total_aligned}\t$h{$f}{single}\t$h{$f}{pct_single}\t$h{$f}{single_aligned_once}\t$h{$f}{single_aligned_multi}\t$h{$f}{pct_overall}\n";
}
